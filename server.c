...
static void handle_connection(int conn_socket)
{
	...
	while(1) {  // Loop indefinitely
		...
		// Block waiting for a new request object from the client
		ssize_t bytes_read = read(conn_socket, &req, sizeof(req));
		if (bytes_read <= 0) {
			perror("Read from socket failed or connection closed");
			break;
		}
		...
		write(conn_socket, &res, sizeof(res));

		// Log
		printf("R%" PRIu64 ":%.6lf,%.6lf,%.6lf,%.6lf\n",
		       req.request_id,
			  ...
			  // TSPEC_TO_DOUBLE(completion_timestamp - req.sent_timestamp));
	}
}
